        ...
        <div class="container-xxl bg-primary page-header" style="background-color: #964B00"
            ...
            <div class="container text-center">
                <h1 style="font-size: 30px" style="font-family: sans-serif" class="text-white animated zoomIn mb-3">
                    Inovasi Pariwisata Kecamatan Delta Pawan Kabupaten Ketapang
                </h1><br>
                {{-- <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-center">
                    </ol> --}}

                <form action="{{ url('Pariwisata') }}" method="post">
                    @csrf
                    <div style="height: 100px" class="position-relative w-50 mt-">
                        <input class="form-control border-0 rounded-pill w-100 ps-4 pe-5" type="text"
                            placeholder="Cari pariwisata" style="height: 25px;" name="judul" id="q"
                            value="{{ $judul ?? '' }}"><br>
            ...
            @foreach ($list_pariwisata as $pariwisata)
                <div class="col-lg-4 wow fadeInUp" data-wow-delay="0.1s">
                    <strong>
                        <article>
                            <div class="card" style="background-size: 10%;" style="width: 25rem;">
                                <br> <img src="{{ "/public/$pariwisata->foto" }}" class="card-img-top" alt="...">
                                <strong>
                                    <div class="card-body" style="background-color: rgb(245, 241, 241)"><br>
                                        <div class="text-overflow">
                                            ...
                                            {{-- <p class="card-text" style="color: black">{{ $pariwisata->isi }}</p><br> --}}

                                            <p style="margin-left: 5%" style="text-align: right" class="btn btn-danger">
                                                {{ $pariwisata->created_at->isoFormat('dddd, D MMMM Y') }}</p>

                                            <a style="margin-left: 2%" href="{{ url('pariwisata', $pariwisata->id) }}"
                    ...
                    {{-- <div class="card shadow border-0">
                                <a href="{{ url('pariwisata', $pariwisata->id) }}">
                                    <div class="carousel-item active">
                                        <img style="width: 100%;" src="{{ "/public/$pariwisata->foto" }}">
                                        <div class="carousel-caption d-none d-md-block"
                                            style="margin-bottom: -20px; height: 50px;">
                                            <p style="opacity: 100%">{{ $pariwisata->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                </a>
                                <div class="card-body">
                                    <p class="card-text">{{ $pariwisata->judul }}</p>
                                </div>
                            </div> --}}
                </div>
        </div>
        @endforeach
        {{-- {!! $list_pariwisata->links() !!} --}}
    </div>
    </div>
    <br><br>
